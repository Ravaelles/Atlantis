...
public class Position {
    ...
    /**
     * Creates a new Position representing the given x and y as Pixel, Walk Tile, or Build Tile coordinates
     * (depending on the PosType given).
     */
     ...
     *
     * Uses Starcraft's approximated distance function, which is reasonably accurate yet avoids a sqrt
     * operation and saves some CPU cycles.
     *
    ...
    /**
     * Returns true if the position is on the map. Note: if map info is unavailable, this function will check
    ...
    /**
     * Returns a <b>new</b> Position in the closest valid map position. Worked out at Build Tile resolution,
     * like in BWAPI 3. Note: if map info is unavailable, this function will check validity against the
     * largest (256x256) map size.
     */
    ...
    /**
     * Returns a <b>new</b> Position that represents the effect of moving this position by delta (treated as a
    ...
    // =========================================================
    /**
     * Returns distance to other position in build tiles. One build tile equals to 32 pixels. Usage of build
    ...
    /**
     * Returns true if it's possible to ground-walk from current position to the other position.
     */
    public boolean hasPathTo(Position otherPosition) {
        return Atlantis.getBwapi().hasPath(this, otherPosition);
    }

    /**
     * Returns true if it's possible to "walk" in this posiiton.
     */
    /**
     * Returns true if it's possible to "walk" in this posiiton.
     */
    public boolean isVisible() {
        return Atlantis.getBwapi().isVisible(this);
    }
