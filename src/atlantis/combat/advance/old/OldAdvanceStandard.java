...
public class OldAdvanceStandard extends MissionManager {
    public OldAdvanceStandard(AUnit unit) {
        super(unit);
    }
...
//            && !ShouldRetreat.shouldRetreat(unit)
            && unit.enemiesNear().canBeAttackedBy(unit, 6).empty()
            && (
            (unit.distToLeader() <= 2 + (A.supplyUsed() / 25.0))
...
//        }
        ...
        if (focusPoint != null) {
            unit.move(focusPoint, Actions.MOVE_ADVANCE, "OldAdvance");
    ...
    public String toString() {
        return "OldAdvanceStandard(" + A.digit(unit.distToLeader()) + ")";
    }
}
