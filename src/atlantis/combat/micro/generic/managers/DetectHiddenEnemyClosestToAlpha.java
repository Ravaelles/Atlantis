...
public class DetectHiddenEnemyClosestToAlpha extends Manager {
    private AUnit dangerousInvisibleEnemy;

    public DetectHiddenEnemyClosestToAlpha(AUnit unit) {
        super(unit);
    }
    ...
    public Manager handle() {
        ...
        if (dist >= 6.2) {
            ...
            unit.moveAwayFrom(dangerousInvisibleEnemy, 0.5, Actions.MOVE_REVEAL, "MoveFromEnemy");
            return usedManager(this);
        }
    }

    private boolean isUnitAssignedForThisTask() {
        return unit.equals(Select.ourOfType(unit.type()).second());
    }
