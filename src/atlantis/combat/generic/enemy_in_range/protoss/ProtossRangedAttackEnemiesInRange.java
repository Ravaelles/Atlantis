...
public class ProtossRangedAttackEnemiesInRange extends Manager {
    ...
    public boolean applies() {
        ...
        if (
            Enemy.zerg()
                && unit.shieldDamageAtLeast(9)
                && unit.distToLeader() > Math.max(5, unit.squadSize())
        ) return false;
        if (unit.hp() <= 60 && unit.meleeEnemiesNearCount(3) > 0) return false;
        if (
            unit.lastAttackFrameLessThanAgo(30 * 5)
                && unit.meleeEnemiesNearCount(1.5 + unit.woundPercent() / 50.0) > 0
        ) return false;
...
//            && unit.lastAttackFrameMoreThanAgo(30 * 2)
            && unit.meleeEnemiesNearCount(3.0 + unit.woundPercent() / 120.0) <= 1;
    }
